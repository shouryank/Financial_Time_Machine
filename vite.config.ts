...
export default defineConfig(({ mode }) => {
  ...
  const setAuthCookie = (res: any, token: string) => {
    ...
    if (mode !== 'development') parts.push('Secure');
        ...
        try {
          if (req.url?.startsWith('/api/auth/signup')) {
            ...
            if (!normalizedEmail || rawPassword.length < 8) {
              return sendJson(res, 400, { error: 'Email and password (min 8 chars) are required' });
            }
            ...
            const insert = await users.insertOne({ email: normalizedEmail, passwordHash, createdAt: now, updatedAt: now });

            const jwtModule = await import('jsonwebtoken');
            const jwt: any = (jwtModule as any).default ?? jwtModule;
            const token = jwt.sign({ email: normalizedEmail }, getJwtSecret(), { subject: insert.insertedId.toString(), expiresIn: `${JWT_TTL_SECONDS}s` });
            setAuthCookie(res, token);
            return sendJson(res, 201, { user: { id: insert.insertedId.toString(), email: normalizedEmail } });
          }
          ...
          if (req.url?.startsWith('/api/auth/login')) {
            ...
            const normalizedEmail = String(email || '').trim().toLowerCase();
            const rawPassword = String(password || '');
            if (!normalizedEmail || !rawPassword) return sendJson(res, 400, { error: 'Email and password are required' });

            const client = await getMongoClient();
            const db = client.db(DB_NAME);
            const user = await db.collection('users').findOne({ email: normalizedEmail }, { projection: { _id: 1, email: 1, passwordHash: 1 } });
            if (!user?.passwordHash) return sendJson(res, 401, { error: 'Invalid credentials' });

            const bcryptModule = await import('bcryptjs');
            const bcrypt: any = (bcryptModule as any).default ?? bcryptModule;
            const isValid = await bcrypt.compare(rawPassword, user.passwordHash);
            if (!isValid) return sendJson(res, 401, { error: 'Invalid credentials' });

            const jwtModule = await import('jsonwebtoken');
            const jwt: any = (jwtModule as any).default ?? jwtModule;
            const token = jwt.sign({ email: user.email }, getJwtSecret(), { subject: user._id.toString(), expiresIn: `${JWT_TTL_SECONDS}s` });
            setAuthCookie(res, token);
            return sendJson(res, 200, { user: { id: user._id.toString(), email: user.email } });
          }
          ...
          if (req.url?.startsWith('/api/auth/me')) {
            ...
            return sendJson(res, 200, { user: { id: session.userId, email: session.email } });
          }
          ...
          if (req.url?.startsWith('/api/plaid/status')) {
            ...
            const user = await db.collection('users').findOne({ _id: new ObjectId(session.userId) }, { projection: { plaidAccessToken: 1, plaidItemId: 1 } });
            return sendJson(res, 200, { connected: Boolean(user?.plaidAccessToken && user?.plaidItemId) });
          }
          ...
          if (req.url?.startsWith('/api/plaid/exchange_public_token')) {
            ...
            if (!public_token) return sendJson(res, 400, { error: 'public_token is required' });
            ...
            await db.collection('users').updateOne(
              ...
              { $set: { plaidAccessToken: exchange.data.access_token, plaidItemId: exchange.data.item_id, plaidUpdatedAt: new Date().toISOString() } }
            );
          ...
          if (req.url?.startsWith('/api/plaid/disconnect')) {
            ...
            const db = client.db(DB_NAME);
            const { ObjectId } = await import('mongodb');
            const user = await db.collection('users').findOne({ _id: new ObjectId(session.userId) }, { projection: { plaidAccessToken: 1 } });
            ...
            await db.collection('users').updateOne(
              { _id: new ObjectId(session.userId) },
              { $unset: { plaidAccessToken: '', plaidItemId: '', plaidUpdatedAt: '' } }
            );
          ...
          const docs = await db.collection('transactions')
            .find(
              ...
              { projection: { _id: 0, id: 1, date: 1, amount: 1, merchant: 1, category: 1, accountId: 1, type: 1, intent: 1, userId: 1 } }
            )
            .sort({ date: 1 })
            .limit(500)
            .toArray();
          return sendJson(res, 200, { transactions: docs });
        } catch (error: any) {
          console.error('API middleware error:', error);
          return sendJson(res, 500, { error: mode === 'development' ? (error?.message || 'Server error') : 'Server error' });
        }
      });
    }
  };
